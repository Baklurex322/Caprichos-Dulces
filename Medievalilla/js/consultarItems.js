...
fetch("http://localhost:3001/").then(recursos => recursos.json()).then(archivo => {
    ...
    for(i = 0; i < arregloItems.length; i++){
        ...
        {
        alert("Borrar la tarjeta:  " + evento.currentTarget.tag); //con evento.currentTarget.tag te dice el elemento actual
...
function botonGuardarCambiosPress(numero)
{
 ...
 function ClickAgregarItm()
 {
     ...
     }).then(respuesta => { //este .then se realiza una vez que toda la funcion terminó
